...
function closeProductModalWindow() {
    ...
    if ($("productDeliveryType")) {
        $("productDeliveryType").value =
            "normal";
    }

    const panel =
        $("conditionalDeliveryPanel");
...
function toggleConditionalDeliveryPanel() {
...
function generateDeliveryOptionId() {

    return (
        `option_${Date.now()}_${Math.random()
            .toString(36)
            .slice(2, 11)}`
    );
}
...
function generateDeliveryFieldId() {
...
function createDeliveryFieldElement(
    ...
    wrapper.className =
        "delivery-field-entry";

    wrapper.dataset.fieldId =
        fieldId;

    wrapper.innerHTML = `
        <div class="delivery-field-inputs">

            <input
                type="text"
                class="field-label-input"
                value="${escapeHTML(fieldLabel)}"
                placeholder="Field Label (e.g., Username)"
                required
            >
            ...
            <input
                ...
                value="${escapeHTML(fieldPlaceholder)}"
                placeholder="Placeholder text"
            >

            <button
                type="button"
                class="admin-action-btn danger delivery-field-delete"
            >
                Remove
            </button>
...
function createDeliveryOptionElement(
    option = {}
) {
    ...
    wrapper.innerHTML = `
        <div class="delivery-option-header">
            ...
            <input
                ...
                value="${escapeHTML(optionName)}"
                placeholder="Option Name (e.g., Steam, Xbox)"
                required
            >

            <button
                type="button"
                class="admin-action-btn danger delivery-option-delete"
            >
                Remove Option
            </button>
        ...
        <div class="delivery-option-fields">
            ...
            <button
                type="button"
                class="admin-secondary-btn add-delivery-field"
            >
                + Add Field
            </button>
...
function getDeliveryConfigFromUI() {
    ...
    optionElements.forEach(
        optionEl => {
            ...
            if (!optionName) {
                ...
                throw new Error(
                    "All delivery options must have a name."
                );
            }

            const fieldElements =
                optionEl.querySelectorAll(
                    ".delivery-field-entry"
                );
            ...
            fieldElements.forEach(
                fieldEl => {
                    ...
                    if (!fieldLabel) {
                        ...
                        throw new Error(
                            `Option "${optionName}": All fields must have a label.`
                        );
                    }
    ...
    if (options.length === 0) {
        ...
        throw new Error(
            "Please add at least one delivery option."
        );
    }
...
function loadDeliveryConfigToUI(
    config = null
) {
    ...
    toggleConditionalDeliveryPanel();
                        ...
                        .map(
                            ([key, value], index) => {
                                ...
                                if (
                                    ...
                                    return `
                                        <section
                                            class="order-delivery-group"
                                        >

                                            <h3
                                                class="order-delivery-group-title"
                                            >
                                                ${escapeHTML(
                                                    gameName
                                                )}
                ...
                (
                    ...
                    categoryRow.innerHTML = `
                        <td colspan="6">
                            ...
                            <span
                                class="product-game-category-label"
                            >
                                GAME CATEGORY
                            </span>
...
const conditionalDeliveryPanel =
    $("conditionalDeliveryPanel");

const deliveryOptionsContainer =
    $("deliveryOptionsContainer");

const addDeliveryOptionBtn =
    $("addDeliveryOptionBtn");


/* ==========================================
   SHOW / HIDE CONDITIONAL DELIVERY
========================================== */
...
function updateDeliveryTypeUI() {
    ...
    if (type === "conditional") {
        ...
        conditionalDeliveryPanel.classList.remove(
            "hidden"
        );

    } else {

        conditionalDeliveryPanel.classList.add(
            "hidden"
        );
    }
}
...
function addDeliveryOption(
    ...
    deliveryOptionsContainer.appendChild(
        optionWrapper
    );
}
...
deliveryOptionsContainer?.addEventListener(
    "click",
    event => {
...
addDeliveryOptionBtn?.addEventListener(
    "click",
    () => {
...
productDeliveryType?.addEventListener(
    "change",
    () => {
...
/* ==========================================
   LOAD DELIVERY CONFIG INTO UI
========================================== */

function setDeliveryConfigUI(
    deliveryConfig
) {
    ...
    if (
        ...
        deliveryConfig.options.forEach(
            option => {
...
/* ==========================================
   RESET DELIVERY CONFIG
========================================== */

function resetDeliveryConfigUI() {
    ...
    async event => {
        ...
        try {
            ...
            try {
                ...
                deliveryConfig =
                    getDeliveryConfigFromUI();

            } catch (error) {

                throw new Error(
                    `Delivery configuration error: ${error.message}`
                );
            }
